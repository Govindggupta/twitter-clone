...
const FollowingFollowers = () => {
  const { username, type } = useParams(); // Extract type from URL
  const navigate = useNavigate(); // To change the URL without a page reload
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 bg-black text-white rounded-md shadow-md">
      {/* Tabs for toggling between followers and following */}
      <div className="flex justify-center space-x-4 border-b border-gray-700 pb-2">
        <button
          className={`px-4 py-2 font-semibold ${
            currentType === 'followers' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-400 hover:text-blue-400'
          }`}
        ...
        <button
          className={`px-4 py-2 font-semibold ${
            currentType === 'following' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-400 hover:text-blue-400'
          }`}
      ...
      {/* Display list of followers or following */}
      {data?.length > 0 ? (
